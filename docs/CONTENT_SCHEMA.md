...
| `issue` | ❌ | 純文字 | |
| `pages` | ❌ | 純文字 | |
| `doi` | 🟡 | DOI URL | v1.4 改為選填；`doi` 與 `article_url` 至少需有一個 |
| `article_url` | 🟡 | URL | v1.4 新增；給無 Crossref DOI 的論文（如 EJHS、學位論文）；`doi` 與 `article_url` 至少需有一個 |
...
**Schema 修正**：
- §七 論文條目：`doi` 從必填改為**選填**
- §七 論文條目：新增 `article_url` 選填欄位（URL 格式）
- **新規則**：論文條目的 `doi` 與 `article_url` **至少需有一個**

**設計動機**：
- 部分歷史/小型 OA 期刊（如 Electronic Journal of Human Sexuality）未註冊 Crossref DOI 系統
- 部分學術產出（學位論文、會議論文、書章）也常無 DOI
- v1.3 強制 doi 必填會逼迫策展人偽造 DOI（已在 williams-2014.md 條目發生）
- v1.4 引入 article_url 作為「無 DOI 時的 stable identifier」備案，符合學術現實
...
**遷移**：v1.3 條目向後相容。既有 6 篇論文有 DOI 的不需改動，僅 williams-2014 需從 doi 移到 article_url。
