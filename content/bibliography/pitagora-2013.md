---
schema_version: "1.4"
title: "Consent vs. Coercion: BDSM Interactions Highlight a Fine but Immutable Line"
authors: "Pitagora, D."
year: 2013
journal: "The New School Psychology Bulletin"
volume: "10"
issue: "1"
pages: "27-36"
doi: "https://doi.org/10.1037/e543732013-004"
access_type: open-access
oa_type: gold
preprint_url: null
study_type: theoretical
methodology_keywords:
  - argumentative
  - conceptual-analysis
sample_size: null
sample_population: null
translation_status: abstract-only
topic_tags: [consent, methodology, legal]
reader_tags: [researcher, clinical, legal-pro]
difficulty: intermediate
abstract_zh: |
  本文聚焦於 BDSM 互動中**同意（consent）與脅迫（coercion）的界線判定**。作者主張，雖然兩者在現象上可能有重疊區（特別在權力交換深度的場景中），但兩者間存在「fine but immutable」（精細但不可動搖）的本質差異。

  論文核心論證包含：(1) 合意 BDSM 互動以雙方持續同意為基礎，脅迫則以一方權力壓制為基礎；(2) BDSM 中的權力交換是「協商過的支配」，與「未協商的支配」（即虐待）有清楚分界；(3) 區分兩者的關鍵指標包括事前協商、安全詞、退場機制、場景外的關係品質。

  本文為臨床工作者與司法從業者提供了區分合意 BDSM 與虐待行為的概念工具，常被用以反駁「BDSM 是合理化虐待」的批評。

curator_note: |
  策展人導讀：本文 open-access 可直接讀全文。對臨床心理師、社工、律師等需要區分「合意 BDSM 動態」與「親密關係暴力」的專業人員特別有參考價值。建議搭配 Williams et al. 2014 的 4Cs 框架一起讀。

caveats:
  - type: methodology
    note: "本文為理論論述，無實證資料。引用時建議搭配實證研究（如 Wismeijer 2013）相互佐證。"
  - type: cultural-context
    note: "論文以美國法律與臨床語境為主，台灣讀者引用時需考慮台灣的法律框架差異。"

replication_status: unknown

citations:
  - type: internal-paper
    ref: williams-2014
    note: "4Cs 同意框架的原典，與本文同屬同意倫理討論脈絡"

contributor: "kinkref"
contributor_note: "本條目由 AI 心理學家 Agent 繆思（Muse）生成"
created_date: 2026-05-02
last_reviewed: 2026-05-02
---

# Consent vs. Coercion: BDSM Interactions Highlight a Fine but Immutable Line

本條目於 frontmatter 中定義。
