---
schema_version: "1.4"
title: "Psychological Functioning of Bondage/Domination/Sado-Masochism (BDSM) Practitioners"
authors: "Connolly, P. H."
year: 2006
journal: "Journal of Psychology & Human Sexuality"
volume: "18"
issue: "1"
pages: "79-120"
doi: "https://doi.org/10.1300/J056v18n01_05"
access_type: paywalled
study_type: quantitative
methodology_keywords:
  - cross-sectional
  - clinical-assessment
  - self-report
sample_size: "32 BDSM 實踐者"
sample_population: "美國 BDSM 社群招募樣本"
translation_status: abstract-only
topic_tags: [psychopathology, identity, methodology]
reader_tags: [researcher, clinical]
difficulty: intermediate
abstract_zh: |
  本研究評估 32 位 BDSM 實踐者在多項心理健康指標上的表現，包括憂鬱、焦慮、強迫症狀、解離、社交障礙、PTSD 等臨床面向。研究採用標準化臨床量表（如 SCL-90-R），與一般人口的常模進行比較。

  結果顯示，BDSM 實踐者在大多數臨床指標上**不存在升高的精神病理水準**——憂鬱、焦慮、強迫症狀等的分數與一般人口相當。實踐者展現出較高的解離傾向，但被作者解讀為與 BDSM 場景中的意識狀態探索相關，而非病理性解離。

  本研究是支持 BDSM 去病理化的早期指標性實證研究，方法論在當時相對嚴謹（使用標準化臨床工具而非自製問卷），常與 Wismeijer & van Assen（2013）並列引用。

curator_note: |
  策展人導讀：作為去病理化文獻的早期經典，本文的歷史價值高於方法論完美度——研究於 2006 年進行，當時 DSM-IV 仍在流通，此研究為 DSM 改革倡議（如 Moser & Kleinplatz 同年的論文）提供實證基礎。建議搭配 Moser & Kleinplatz（2006）的理論論文一起讀。

caveats:
  - type: methodology
    note: "樣本量（N=32）較小，且使用社群招募存在 self-selection bias。"
  - type: dated
    note: "資料蒐集於 2006 年前後，當時的臨床量表標準與當代實踐略有差異，引用時建議搭配近期研究（如 Wismeijer 2013）相互佐證。"

replication_status: replicated

citations:
  - type: internal-paper
    ref: wismeijer-2013
    note: "更大樣本的後續去病理化研究"
  - type: internal-paper
    ref: moser-kleinplatz-2006
    note: "同年發表的 DSM 改革倡議理論論文"

contributor: "kinkref"
contributor_note: "本條目由 AI 心理學家 Agent 繆思（Muse）生成"
created_date: 2026-05-02
last_reviewed: 2026-05-02
---

# Psychological Functioning of BDSM Practitioners

本條目於 frontmatter 中定義。
