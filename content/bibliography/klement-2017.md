---
schema_version: "1.4"
title: "Extreme Rituals in a BDSM Context: The Physiological and Psychological Effects of the 'Dance of Souls'"
authors: "Klement, K. R.; Lee, E. M.; Ambler, J. K.; Hanson, S. A.; Comber, E.; Wietting, D.; Wagner, M. F.; Burns, V. R.; Cutler, B.; Cutler, N.; Reid, E.; Sagarin, B. J."
year: 2017
journal: "Culture, Health & Sexuality"
volume: "19"
issue: "4"
pages: "453-469"
doi: "https://doi.org/10.1080/13691058.2016.1234648"
access_type: paywalled
study_type: quantitative
methodology_keywords:
  - field-study
  - hormonal-measurement
  - pre-post-design
  - psychometric-assessment
sample_size: "Dance of Souls 儀式參與者（小樣本現場研究）"
sample_population: "美國 'Dance of Souls' 儀式參與者，含 hook pull 與 flesh hook suspension 等高強度 practice"
translation_status: abstract-only
topic_tags: [neuroscience, practice-specific, methodology]
reader_tags: [researcher, clinical]
difficulty: advanced
abstract_zh: |
  本研究延續 Sagarin 等（2009）的場景生理測量研究方向，將焦點放在**極端 BDSM 儀式**（Extreme rituals）的生理與心理效應。研究對象為 'Dance of Souls' 儀式參與者，該儀式包含 hook pull（鉤拉）、flesh hook suspension（皮膚鉤懸吊）等高強度 practice。

  **主要發現**：(1) 儀式參與者在生理層面展現皮質醇上升、α-amylase 上升等壓力反應；(2) 心理層面出現**意識狀態改變**（altered states of consciousness）的指標——時間感扭曲、自我邊界融解、強烈情緒釋放等，與 subspace 的主觀描述相符；(3) 儀式後參與者在 PANAS（正負情緒量表）上展現顯著的正面情緒提升。

  本研究是 BDSM 神經科學中**極端強度場景**的少數實證資料，為 subspace 與 transcendent state（超越意識狀態）的連結提供初步生理基礎。研究方法論延續 Sagarin 系列的嚴謹現場測量風格。

curator_note: |
  策展人導讀：本文聚焦於極端 BDSM 儀式，結果未必適用於一般場景，但其方法論與生理測量框架對任何 subspace 研究都有參考價值。建議先讀 Sagarin 等（2009）建立基礎，再讀本文了解延伸。

caveats:
  - type: methodology
    note: "樣本來自特定儀式社群，研究結果不適合推論到一般 BDSM 場景的生理反應。"
  - type: cultural-context
    note: "'Dance of Souls' 儀式有特定的美國加州 fetish 文化脈絡，跨文化引用時需注意儀式形式的差異。"

replication_status: unknown

citations:
  - type: internal-paper
    ref: sagarin-2009
    note: "前驅研究，建立了場景生理測量的方法論基礎"

contributor: "kinkref"
contributor_note: "本條目由 AI 心理學家 Agent 繆思（Muse）生成"
created_date: 2026-05-02
last_reviewed: 2026-05-02
---

# Extreme Rituals in a BDSM Context

本條目於 frontmatter 中定義。
