---
schema_version: "1.4"
term_zh: 個人責任知情同意 kink
term_en: PRICK
aliases_en: Personal Responsibility Informed Consensual Kink
tagline: 強調個人責任的當代 BDSM 同意框架。
definition: |
  PRICK（Personal Responsibility Informed Consensual Kink）是 BDSM 同意框架的進一步發展，作為 SSC 與 RACK 的延伸。PRICK 的核心主張是：每個參與者對自身的決定承擔最終責任，包括參與該 practice 的選擇、對風險的認識、以及對自身狀態的監測。

  PRICK 強調四個面向：Personal Responsibility（個人責任）、Informed（知情）、Consensual（同意）、Kink（在合意 kink 框架內）。這個框架特別適用於 edge play 等高風險 practice——當風險無法消除時，每個參與者必須清楚知道自己在做什麼決定。

  PRICK 並未取代 RACK，而是補充：RACK 強調雙方對風險的共同認知，PRICK 強調個人對自身決定的責任。兩個框架可以並用，互相補強。

disambiguation:
  - vs: SSC
    note: |
      SSC 強調「安全」「理智」的判定，PRICK 強調個人對自身決定的責任。PRICK 假設參與者具備為自己決定負責的能力。
  - vs: RACK
    note: |
      RACK 強調雙方對風險有充分認識，PRICK 強調個人對自身決定的責任。兩者方向相同（強調認識而非絕對安全），但 PRICK 將最終責任明確歸於個人。

usage_notes: |
  在台灣中文圈，PRICK 較少作為獨立框架使用，多在介紹 RACK 時被一併提及。實踐中，PRICK 的精神（個人責任）已融入 RACK 與 4Cs 等框架的實踐邏輯，較少作為單獨的標籤使用。

origin_year: "2000s"
related_terms: [ssc, rack, safe-word, hard-limit, soft-limit]

external_references:
  - source: wikipedia
    url: "https://en.wikipedia.org/wiki/BDSM#PRICK"

topic_tags: [consent, history]
reader_tags: [practitioner, researcher, educator]
contributor: "kinkref"
contributor_note: "本條目由 AI 心理學家 Agent 繆思（Muse）生成"
created_date: 2026-05-02
last_reviewed: 2026-05-02
---

# PRICK（個人責任知情同意 kink）

本條目於 frontmatter 中定義。
