---
schema_version: "1.4"
title: "Electronic Journal of Human Sexuality"
title_zh: null
abbreviation: "EJHS"
issn_online: "1545-5556"
publisher: "Institute for Advanced Study of Human Sexuality"
founded: 1998
website: "http://www.ejhs.org/"
language: en

scope: |
  歷史期刊（1998-2014）。期刊涵蓋人類性學的廣泛主題，收錄論文、學位論文、海報、評論等多種學術產出形式。編輯方針開放，是早期數位 OA 學術出版的先驅之一，特別歡迎非主流主題（含 BDSM、同志研究、跨性別、性少數）的學術論述。

accepts_bdsm: yes-frequent
bdsm_history: |
  EJHS 在其活躍期是 BDSM 學術論述的重要平台。本站收錄的 **Williams, Thomas, Prior & Walters (2014)** 「From SSC and RACK to the 4Cs」即發表於本期刊。期刊歷史上累積了相當規模的 kink、polyamory、同意框架、性少數實踐等主題的開放取用論文。

access_type: diamond-oa
ceased_year: 2015
ceased_note: "已停刊 ~2015；域名 ejhs.org DNS 已死，Wayback Machine 已存檔"
trust_level: "低（但 Williams et al. 2014 4Cs 論文被引用 150+）"

abstract_zh: |
  **Electronic Journal of Human Sexuality（EJHS）** 是早期開放取用學術期刊的代表之一，由 Institute for Advanced Study of Human Sexuality 主辦，1998 年至 2014 年共發行 17 卷。期刊是 diamond OA（讀者作者皆免費），對 BDSM 與性少數議題持開放立場。

  **本期刊已於 2014 年停止接收新投稿並停刊**，但 archives 仍公開可訪問。對研究者而言，EJHS 的歷史 archives 包含許多早期 BDSM 同意框架、性少數研究、性教育論述的重要文獻——本站收錄的 Williams 等 (2014) 4Cs 框架原典即發表於此。

curator_note: |
  策展人註：本期刊已停刊（2014），無法投稿，但歷史 archives 仍是 BDSM 早期論述的重要參考。引用前注意確認 archives 連結仍有效。當代類似定位的開放取用平台可考慮 PsyArXiv 預印本或 PLOS ONE。

caveats:
  - type: dated
    note: "本期刊於 2014 年停刊，無法投稿新論文。Archives 仍公開可訪問，但連結穩定性需驗證。"

citations:
  - type: internal-paper
    ref: williams-2014
    note: "本站收錄論文，發表於本期刊（停刊前最後幾批）"

topic_tags: [history, gender-sexuality, methodology]
reader_tags: [researcher]
contributor: "kinkref"
contributor_note: "由 AI 心理學家 Agent 繆思（Muse）協助撰寫，經策展人薩約審校"
created_date: 2026-05-03
last_reviewed: 2026-05-04
reviewer: "策展人：薩約"
---

# Electronic Journal of Human Sexuality（已停刊）

本條目於 frontmatter 中定義。
